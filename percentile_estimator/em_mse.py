...
class EMMSE(Estimator):

    # noisy max (equivalent to EM but faster): query is q(D,\ell) = -(n - n_{\ell} + std)
    def obtain_ell(self, p=0):
        return primitive.em(self.quality_func(), self.epsilon, sensitivity=1, monotonic=True) * self.args.s
    ...
    def quality_func(self):
        ...
        # noise_var = 4 * (16 - 1) * num_levels ** 3 / self.args.range_epsilon ** 2
        # noise_std = noise_var ** 0.5 * self.users.m / (self.users.n - self.users.m)
